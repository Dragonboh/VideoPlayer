...
- (void)playVideoWithTitle:(NSString *)title URL:(NSURL *)url videoID:(NSString *)videoID shareURL:(NSURL *)shareURL isStreaming:(BOOL)streaming playInFullScreen:(BOOL)playInFullScreen isAlwaysFullScreen:(BOOL)alwaysFullScreen
{
    ...
    self.isAlwaysFullscreen = alwaysFullScreen;
    ...
    [self.videoPlayerView setBackgroundColor:[UIColor greenColor]];
    [self.videoPlayerView.airplayIsActiveView setBackgroundColor:[UIColor blackColor]];
}
...
- (void)setPreviousBounds:(CGRect)previousBounds
{
    ...
    if (self.fullScreenModeToggled) {
        ...
        if (self.isAlwaysFullscreen) {
            ...
            [UIView animateWithDuration:0.45f
                                ...
                                options:UIViewAnimationCurveLinear
                             animations:^{
                                 self.videoPlayerView.frame = CGRectMake(self.previousBounds.origin.x, self.previousBounds.origin.y, self.previousBounds.size.width, self.previousBounds.size.height/2);
                             }
                             completion:^(BOOL success){
                                 if (showShareOptions) {
                                     [self presentShareOptions];
